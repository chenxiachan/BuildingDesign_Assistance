...
if(LOD_STAGE == 1):
    ...
    cat_dict = {
        ...
        # 'MAINHEAT_DESCRIPTION': {'Air source heat pump, radiators, electric': 0, 'Boiler and radiators, LPG': 1, 'Boiler and radiators, anthracite': 2, 'Boiler and radiators, bottled gas': 3, 'Boiler and radiators, dual fuel (mineral and wood)': 4, 'Boiler and radiators, electric': 5, 'Boiler and radiators, mains gas': 6, 'Boiler and radiators, oil': 7, 'Boiler and radiators, wood logs': 8, 'Boiler and underfloor heating, LPG': 9, 'Boiler and underfloor heating, electric': 10, 'Boiler and underfloor heating, mains gas': 11, 'Boiler and underfloor heating, oil': 12, 'Community scheme': 13, 'Community scheme with CHP': 14, 'Community scheme, mains gas': 15, 'Electric ceiling heating': 16, 'Electric storage heaters': 17, 'Electric underfloor heating': 18, 'Ground source heat pump, radiators, electric': 19, 'Ground source heat pump, underfloor, electric': 20, 'No system present: electric heating assumed': 21, 'Portable electric heating assumed for most rooms': 22, 'Room heaters, coal': 23, 'Room heaters, dual fuel (mineral and wood)': 24, 'Room heaters, electric': 25, 'Room heaters, mains gas': 26, 'Room heaters, smokeless fuel': 27, 'Room heaters, wood logs': 28, 'Warm air, Electricaire': 29, 'Warm air, electric': 30, 'Warm air, mains gas': 31}
        ...
        'WALLS_DESCRIPTION': {'Cavity wall, as built, insulated': 0, 'Cavity wall, as built, no insulation': 1, 'Cavity wall, filled cavity': 2, 'Cavity wall, with external insulation': 3, 'Cavity wall, with internal insulation': 4, 'Cob, as built': 5, 'Granite or whinstone, as built, insulated': 6, 'Granite or whinstone, as built, no insulation': 7, 'Granite or whinstone, with external insulation': 8, 'Granite or whinstone, with internal insulation': 9, 'Sandstone, as built, insulated': 10, 'Sandstone, as built, no insulation': 11, 'Sandstone, with internal insulation': 12, 'Solid brick, as built, insulated': 13, 'Solid brick, with external insulation': 14, 'Solid brick, with internal insulation': 15, 'System built, as built, insulated': 16, 'System built, with external insulation': 17, 'System built, with internal insulation': 18, 'Timber frame, as built, insulated': 19, 'Timber frame, with internal insulation': 20},
...
def point_prediction(SUBJECT, model, mu_list, sigma_list, y_dists, input_f):
    ...
    with st.expander(SUBJECT + '\'s detail'):
    ...
    if(length >= 8000000):
        ...
        with st.expander('Suggestions:'):
            ...
            for i in df_impact.columns:
                ...
                st.markdown(text)
                st.code(df_estimation.loc[df_impact[i].idxmax()])
            st.markdown('-')
            for i in df_impact.columns:
                ...
                st.markdown(text)
                st.code(df_estimation.loc[df_impact[i].idxmin()])
...
with st.expander('Encoding Dictionary：'):
